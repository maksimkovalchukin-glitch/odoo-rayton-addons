...
# ── Exact options from webapp HTML ────────────────────────────────────────────

# module_type: value = display text (sent to n8n); watts extracted from text
MODULE_TYPES = [
...
def _get_module_watts(module_type_text):
    """Extract watt number from text like 'Trina 575W' or 'потужністю 625W' → watts as float kW."""
...
class KpGenerateWizard(models.TransientModel):
    ...
    # ── Common ────────────────────────────────────────────────────────────────
    sale_order_id = fields.Many2one('sale.order', string='Пропозиція',
    ...
    manager_id   = fields.Many2one('res.users', string='Менеджер', required=True)
    ...
    module_type       = fields.Selection(MODULE_TYPES,   string='Тип панелей')
    mount_type        = fields.Selection(MOUNT_TYPES,    string='Тип монтажу')
    material_type     = fields.Selection(MATERIAL_TYPES, string='Матеріали', default='DC та AC')
    ses_type          = fields.Selection(SES_TYPES,      string='Тип СЕС',   default='Дахова')

    # Fixed values — shown readonly, always sent as-is
    power_regulation  = fields.Char(
        string='Регулювання потужності',
        default='Лічильник + Трансформатори струму (3шт)',
        readonly=True,
    )
    monitoring_device = fields.Char(
        string='Пристрій моніторингу',
        default='Huawei Smart Dongle',
        readonly=True,
    )
    price_vat_type    = fields.Char(
        string='Ціна (ПДВ)',
        default='з ПДВ',
        readonly=True,
    )
    price_per_kw = fields.Float(string='Ціна за кВт', digits=(10, 2))

    # Consumption mode
    monthly_consumption = fields.Float(
        string='Місячне споживання (МВт·год/міс)', digits=(10, 2))

    # Power mode
    planned_dc_power = fields.Float(
        string='Планова потужність DC (кВт)', digits=(10, 2))
    ...
    # Manual mode — live DC/AC hint (mirrors webapp real-time feedback)
    manual_ratio_info = fields.Char(
        string='DC/AC розрахунок',
        compute='_compute_manual_ratio_info',
        store=False,
    )

    # ── УЗЕ ──────────────────────────────────────────────────────────────────
    uze_model     = fields.Selection(UZE_MODELS, string='Модель УЗЕ')
    uze_qty       = fields.Integer(string='Кількість (1–50)', default=1)
    uze_vat       = fields.Selection([('без ПДВ', 'без ПДВ'), ('з ПДВ', 'з ПДВ')],
                                     string='Вартість УЗЕ', default='без ПДВ')
    equipment_vat = fields.Selection([('з ПДВ', 'з ПДВ'), ('без ПДВ', 'без ПДВ')],
    ...
    @api.depends(
        'inverter_1_model', 'inverter_1_qty',
        'inverter_2_model', 'inverter_2_qty',
        'inverter_3_model', 'inverter_3_qty',
        'panel_qty_manual', 'module_type', 'ses_mode',
    )
    def _compute_manual_ratio_info(self):
        inv_power_map = {i['name']: i['power'] for i in INVERTERS}

        for rec in self:
            if rec.ses_mode != 'manual':
                rec.manual_ratio_info = ''
                continue
            ...
            real_ac = (
                pwr(rec.inverter_1_model, rec.inverter_1_qty) +
                pwr(rec.inverter_2_model, rec.inverter_2_qty) +
                pwr(rec.inverter_3_model, rec.inverter_3_qty)
            )

            module_kw = _get_module_watts(rec.module_type)
            real_dc = rec.panel_qty_manual * module_kw if module_kw else 0.0
            ...
            if real_ac > 0 and real_dc > 0:
                ...
                rec.manual_ratio_info = (
                    f'AC: {real_ac:.0f} кВт  |  '
                    f'Оберіть панелі та вкажіть кількість для розрахунку DC'
                )
            else:
                rec.manual_ratio_info = 'Оберіть інвертори та вкажіть кількість панелей'
    ...
    def action_generate(self):
        ...
        try:
            ...
            raise UserError(
                ...
                f'Перевірте підключення та налаштування n8n.'
            )
    ...
    def _build_ses_payload(self):
        mode = self.ses_mode
        module_kw = _get_module_watts(self.module_type)

        if mode == 'consumption':
            payload = self._calc_consumption(module_kw)
        elif mode == 'power':
            payload = self._calc_power(module_kw)
        elif mode == 'roof':
            payload = self._calc_roof(module_kw)
        else:
        ...
        payload.update({
            ...
            'module_type':       self.module_type or '',
    ...
    def _calc_consumption(self, module_kw):
        mwh = self.monthly_consumption
        if mwh < MIN_MONTHLY_MWH:
            raise UserError(f'Місячне споживання має бути не менше {MIN_MONTHLY_MWH} МВт·год.')

        target_ac = _round_to_50((mwh * 1000 / GENERATION_PER_100KW) * 100)
        ...
        return {
            ...
            'panel_qty': panel_qty,
            **_build_inverter_payload(inv),
        }

    def _calc_power(self, module_kw):
        dc = self.planned_dc_power
        if dc <= 0:
            raise UserError('Вкажіть планову потужність DC (кВт).')

        target_ac = _round_to_50(dc / DC_AC_RATIO)
        target_ac = max(target_ac, MIN_AC_KW)
    ...
    def _calc_manual(self):
        ...
        real_ac = (
            pwr(self.inverter_1_model, self.inverter_1_qty) +
            pwr(self.inverter_2_model, self.inverter_2_qty) +
            pwr(self.inverter_3_model, self.inverter_3_qty)
        )
        if real_ac < 30:
            raise UserError('Загальна AC потужність інверторів має бути не менше 30 кВт.')

        module_kw = _get_module_watts(self.module_type)
        real_dc = self.panel_qty_manual * module_kw if module_kw else 0.0

        if real_ac > 0 and real_dc > 0:
            ratio = real_dc / real_ac
            if not (MIN_RATIO <= ratio <= MAX_RATIO):
                raise UserError(
                    f'DC/AC = {ratio:.2f} — виходить за норму ({MIN_RATIO}–{MAX_RATIO}).'
                )
    ...
    def _build_uze_payload(self):
        ...
        if max_qty and self.uze_qty > max_qty:
            raise UserError(
                f'Для моделі "{self.uze_model}" max кількість: {max_qty}.'
            )
        return {
            'project_name':   self.project_name,
            'manager':        self.manager_id.name or '',
